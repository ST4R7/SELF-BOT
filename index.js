...
* Tolong jan di jual belikan nih sc ajg , ini free!
* jan lupa bintang ;( buat support : https://github.com/Zalleya
*mau nanya? pm : 081344291903
*/
...
//onst setting = JSON.parse(fs.readFileSync('./src/setting.json'))
...
prefix = '' //ini prefix ubah aja
fake = '*mengakak ajg*' //ini buat fake reply
numbernye = '0' //nomer reply
targetprivate = '6281344291903' //kalo private chat rply
blocked = []            
...
function kyun(seconds){
  ...
  return `❏RUN TIME ${pad(hours)}H ${pad(minutes)}M ${pad(seconds)}S`
}
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\ Welcome to group *${mdata.subject}* jan lupa intro ya kak :v`
				...
				teks = `Sayonaraaa @${num.split('@')[0]}👋 udah gw bilangin g bakal betah.`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, teks, MessageType.text)
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const barbarkey = 'memek00'   //ubah ke apikey lu
			const vhtearkey = 'fauzan2k19' //ubah ke apikey lu
			...
			mess = {
				wait: 'Wait...',
				success: 'alhamdulillah bisa :v',
				error: {
					stick: 'lah itu video goblok',
					Iv: 'Linknya mokad:v'
				},
				only: {
					...
					Badmin: 'Jadiin gw admin dlu lah'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["6281344291903@s.whatsapp.net"] // ganti nomer lu
			...
			switch(command) {
					...
					case 'loli':
						{
							...
							axios.get(url)
							  .then((result) => {
								...
								.then(
									(response) => {
										client.sendMessage(from, 'Wait...', MessageType.text, { quoted: mek } )
							var buf = Buffer.from(response, 'base64'); 
				...
				client.sendMessage(from, `Wait...`, MessageType.text, tahta)
				client.sendMessage(from, buffer, MessageType.image, { caption: `noh udh jadi...`, quoted: mek })
				 break
				 case 'map':
					var itsme = `${numbernye}@s.whatsapp.net`
					var split = `${fake}`
					...
					.then((res) => {
					  imageToBase64(res.data.gambar)
						.then(
						  (ress) => {
							client.sendMessage(from, 'Wait...', MessageType.text, maping)
							var buf = Buffer.from(ress, 'base64')
							client.sendMessage(from, buf, MessageType.image, { caption: `noh udh jadi...`, quoted: mek })
						})
					})
					break
				 case 'thunder':
				...
				client.sendMessage(from, `Wait...`, MessageType.text, thunder)
				client.sendMessage(from, buffer, MessageType.image, { caption: `noh udh jadi..`, quoted: mek })
					 break
					 case 'otakulast':
...
◪ Advance Menu
  ...
  ├─ ❏ $(prefix)cekchat
  └─ ❏ $(prefix)blocklist
◩ 𝗦𝗲𝗹𝗳 𝗕𝗼𝘁`, MessageType.text, bruhhhh)
	break
case 'tomp3':
...
case 'ping':
	...
	client.sendMessage(from, `${teks}❏SPEED: ${latensi.toFixed(4)}'S`, MessageType.text, pingbro)
	})
	break
	break
case 'term':
...
/*
*Thanks For 𝗠𝗵𝗮𝗻𝗸𝗕𝗮𝗿𝗕r
*Mau recode ya:) silahkan bang
*jan lupa foloow ig _fauzan2k19
*/
